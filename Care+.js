/* =========================================================
   API HELPER
   ========================================================= */
const API_BASE = '/api'; // satu server sama frontend, jadi path relatif aja
...
/* =========================================================
   STATE — sekarang hasil fetch dari server, bukan localStorage
   ========================================================= */
let doctors = [];
let profiles = [];
let bookings = [];
...
async function loadDoctors() { doctors = await api('/doctors'); }
async function loadProfiles() { profiles = await api('/profiles'); }
async function loadBookings() { bookings = await api('/bookings'); }
...
function updateStats(){
  ...
  const sp = document.getElementById('stat-profiles');
  if(sd) sd.textContent = doctors.length;
  if(sb) sb.textContent = bookings.length;
  if(sp) sp.textContent = profiles.length;
}
...
function renderDoctors(){
  ...
  const homeList = document.getElementById('home-doctors');
  const searchInput = document.getElementById('doctor-search');
  const keyword = (searchInput?.value || '').toLowerCase().trim();

  const filtered = keyword
    ? doctors.filter(d => d.name.toLowerCase().includes(keyword) || d.spec.toLowerCase().includes(keyword))
    : doctors;

  if (doctorList) {
    doctorList.innerHTML = filtered.length
      ? filtered.map(doctorCardHTML).join('')
      : `<div class="card empty">Dokter dengan kata kunci tersebut tidak ditemukan.</div>`;
  }

  if (homeList) {
    homeList.innerHTML = doctors.filter(d => d.avail === 'yes').slice(0,4).map(doctorCardHTML).join('');
  }

  const selectDoc = document.getElementById('b-doctor');
  if (selectDoc) {
    const prevValue = selectDoc.value;
    selectDoc.innerHTML = '<option value="">Pilih Dokter</option>' +
      doctors.map(d => `<option value="${d.id}">${esc(d.name)} - ${esc(d.spec)}</option>`).join('');
    if (prevValue) selectDoc.value = prevValue;
  }
    ...
    return `
      <div class="card" style="position:relative; border-top:4px solid ${p.isMain?'var(--primary)':'var(--muted)'}; padding:20px; background:#fff;">
        ...
        <div style="display:flex; justify-content:flex-end; gap:8px; margin-top:12px; padding-top:8px;">
          ...
          ${!p.isMain ? `<button class="btn btn-danger btn-sm" onclick="deleteProfileData(${p.id})">Hapus</button>` : ''}
        </div>
      </div>`;
  }).join('');
...
function deleteProfileData(id) {
  openConfirmModal('Hapus profil anggota keluarga ini? Semua data identitas medis lama akan dilepas dari riwayat cetak.', async () => {
    try {
...
document.getElementById('profile-form').addEventListener('submit', async function(e) {
  ...
  const payload = { name, birth, gender, phone, email, nik, kk, passport };

  try {
    if (id) {
      await api(`/profiles/${id}`, { method: 'PUT', body: JSON.stringify(payload) });
...
function deleteBooking(id){
  openConfirmModal('Hapus rekam janji medis terintegrasi ini secara permanen dari sistem?', async ()=>{
    try {
      await api(`/bookings/${id}`, { method: 'DELETE' });
      await loadBookings();
      renderIntegratedHistory(); renderDoctors();
...
bookingForm.addEventListener('submit', async e=>{
  ...
  const payload = {
    profileId, doctorId, hospital, date, time,
    temp: temp ? Number(temp) : null, symptom, history,
    fallbackAge: activeProf ? getAge(activeProf.birth) : 20
  };

  try {
    if (editId) {
      await api(`/bookings/${editId}`, { method: 'PUT', body: JSON.stringify(payload) });
    ...
    await loadBookings();
    bookingForm.reset(); clearErrors('booking-form');
...
async function init(){
  try {
    await Promise.all([loadDoctors(), loadProfiles(), loadBookings()]);
    renderDoctors();
    renderProfiles();
    renderIntegratedHistory();
  } catch (err) {
    console.error('Gagal load data dari server:', err);
    toast('Gagal terhubung ke server. Pastikan backend sudah jalan (npm start).');
  }
}
init();
